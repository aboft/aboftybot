...
function formatDate(date) {
  ...
  if (date) {
    date.toString().match(/^[0-9][0-9][0-9][0-9]-/)
      ? (date = date + "T00:00")
      : (date = date);
  }
  var d = new Date(date),
...
const setLineCount = async (channel) => {
  const createChannelCount = await knex("line_counts").insert({
    channel: channel,
    count: 1,
  ...
  console.log(
    `CREATED NEW ENTRY IN DB FOR ${channel} ON ${formatDate(new Date())}`
  );
};

const updateLineCount = async (channel) => {
  let channelExists = await knex("line_counts")
    .where({ channel, dateCreated: formatDate(new Date()) })
    .select("channel");
  if (channelExists.length < 1) {
    console.log(
      `${channel} NOT FOUND IN DB. CREATING NEW ENTRY FOR ${channel} ON ${formatDate(
        new Date()
      )}`
    );
    ...
    await knex("line_counts")
      ...
      .increment("count", 1);
    console.log(
      `UPDATING LINE COUNT FOR ${channel} ON ${formatDate(new Date())}`
    );
  }
};
...
const getLineCount = async (channel, dateCreated) => {
  let dateSearch;
  dateCreated
    ? (dateSearch = formatDate(dateCreated))
    : (dateSearch = formatDate(new Date()));
  const lineCount = await knex("line_counts")
    .where({ channel, dateCreated: dateSearch })
    .select("count");
  if (lineCount.length < 1) {
    return `Unable to find lines said for date: ${
...
const getMaxLines = async (channel) => {
...
const checkLineCount = async (channel) => {
  let numOfLines;
  const numOfLinesQuery = await knex("line_counts").where({
    channel: channel,
    dateCreated: formatDate(new Date()),
  });
