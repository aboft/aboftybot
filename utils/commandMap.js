...
let map = new Map([
    ['.testmedaddy', (from, to, text) => (`${text.split(" ")[1] || from}, I stg I'll tie you down...`)],
    ['.cough', (from, to, text) => (`${text.split(' ')[1] || from} Get 6 feet back, you fucking heathen...`)],
    ['.gnulag', (from, to, text) => (`${text.split(' ')[1] || from}, learn to spell gulag correctly, you pleb!`)],
    ['.alacritty', (from, to, text) => (`${text.split(' ')[1] || from}, wtf is alacritty? All I know is I got alldemtitties.`)],
    ['.cobol', (from, to, text) => (`(${text.split(' ')[1] || from}) ${cobol[Math.floor(Math.random() * cobol.length)]}`)],
    ['.covid', async (from, to, text) => {
        const covidCases = await getCovidCases(text.split(' ').slice(1).join(' '))
        return (`${from}, ${covidCases}`)
    }],
    ['.lines', async (from, to, text) => {
        const numOfLines = await getLineCount(to, text.slice(7))
        return (`(${from}), ${numOfLines}`)
    }],
    ['.active', async (from, to, text) => {
