...
const checkUserActive = async (user) => {
  const userExists = await knex("active_user")
    .where({ user })
    .select("message", "lastModified");
  if (userExists.length < 1) {
    return `I have not seen ${user} talking in this channel.`;
  } else {
    console.log(`RETRIEVED USER ${user} FROM active_user TABLE.`);
    const lastTalk =
      (new Date() - new Date(userExists[0]["lastModified"])) / 1000;
    ...
    if (days > 0) {
      return `${user} was last seen talking ${maybePluralize(
        days,
        "day"
      )} and ${
        hours > 0
          ? maybePluralize(hours, "hour")
          : maybePluralize(minutes, "minute")
      } ago saying "${userExists[0]["message"]}"`;
    } else if (hours > 0) {
      return `${user} was last seen talking ${maybePluralize(
        hours,
        "hour"
        ...
        minutes > 0
          ? maybePluralize(minutes, "minute")
          : maybePluralize(sec, "second")
      } ago saying "${userExists[0]["message"]}"`;
    } else {
      return `${user} was last seen talking ${maybePluralize(
        minutes,
        "minute"
      )} and ${maybePluralize(sec, "second")} ago saying "${
        userExists[0]["message"]
      }"`;
    }
  }
};
...
const updateActiveUserMessage = async (user, message) => {
  const isUserActive = await knex("active_user")
    .where({ user })
    .select("message", "lastModified");
