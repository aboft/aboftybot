...
const createInsult = async (user, insult) => {
  const insultExists = await knex
    .select("insult")
    .where("insult", insult)
    .from("gtfb");
  if (insultExists.length < 1) {
    console.log(
      `CREATING INSULT: ${insult} IN aboftybot.gtfb TABLE FOR ${user}`
    );
    const newInsult = await knex("gtfb").insert({
      id: "null",
      insult,
      createdBy: user,
      dateCreated: new Date(),
    });
    const insultId = await knex
      .select("id")
      .where("insult", insult)
      .from("gtfb");
...
const selectRandomInsult = async (id) => {
  if (parseInt(id)) {
    ...
    const insult = await knex
      ...
      .limit(1);
    console.log(
      `SUCCESSFULLY RETRIEVED ${insult[0]["insult"]} FROM aboftybot.gtfb`
    );
    return `${insult[0]["insult"]}`;
  }
};
...
const deleteInsult = async (user, id) => {
  const insultOwner = await knex
    .select("createdBy")
    .where("id", id)
    .from("gtfb");
  if (
    (insultOwner.length > 0 && insultOwner[0]["createdBy"] == user) ||
    user == "aboft"
  ) {
...
const findInsult = async (id) => {
  ...
  const insult =
    insultId.length > 0 ? insultId[0]["insult"] : "Unable to find insult.";
  return insult;
};
